...
void alvarCallback_2(const ar_track_alvar_msgs::AlvarMarkers::ConstPtr& msg) {   
	if (empty) {
		ROS_INFO("The wrist camera cannot see the tag, use overhead camera instead...");
		if (!msg->markers.empty()) {
			geometry_msgs::Pose p;
...
int main(int argc, char **argv)
{
	...
	// subscribe to the left overhead camera topic if wrist camera cannot see the tag

	ros::CallbackQueue vision_queue_2;
	ros::SubscribeOptions opt_2 = ros::SubscribeOptions::create<ar_track_alvar_msgs::AlvarMarkers>(
			"/ar_pose_marker_2",
			100,
			&alvarCallback_2,
			ros::VoidPtr(),
			&vision_queue_2);
	ros::Subscriber vision_sub_2 = node_handle.subscribe(opt_2); 

	ros::AsyncSpinner spinner_2(1, &vision_queue_2);
	spinner_2.start();
